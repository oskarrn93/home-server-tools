...
A Docker Compose stack for a local AI/LLM setup: Ollama (model runtime), LiteLLM (OpenAI-compatible proxy in front of local and desktop Ollama, with fallbacks/caching), SearXNG (private search, used as a web-search backend), Open WebUI (chat UI), and n8n plus its sandbox service (workflow automation with an AI code-execution sandbox). Previously part of the sibling `media-services` repo, moved here since it isn't media-specific. Almost everything is configuration (`docker-compose.yml`, `.env` files, YAML) rather than application code.
...
- **ollama**: model runtime, no web UI, HTTP API on `11434` - published on `127.0.0.1` only (no auth), for `terraform/`'s ollama provider. `ollama/Modelfile_qwen_2_5_3b_instruct` is bind-mounted at `/Modelfile`. Local models LiteLLM needs are pulled via `ollama_model` resources in `terraform/main.tf`.
- **litellm**: OpenAI-compatible proxy (`litellm/config.yaml`) used by Open WebUI, n8n, and media-services apps (Immich, Paperless) over `media-services_default`. Prefers the desktop GPU host's Ollama and falls back to local models. Postgres/Valkey on the host, SSO via Pocket ID.
...
- **openwebui**: chat frontend, depends on `ollama` and `searxng` being healthy; talks to both Ollama directly and LiteLLM.
- **n8n**: depends on `ollama`, `searxng`, and `n8n-sandbox-api` being healthy. Wires its own AI sandbox provider to `n8n-sandbox-api` and its default LLM to LiteLLM (`N8N_INSTANCE_AI_MODEL_URL: http://litellm:4000/v1`). Its host port is bound to `172.17.0.1` only, for `terraform/n8n.tf`'s REST API calls. Workflows are managed by Terraform (`terraform/n8n.tf`, `terraform/workflows/`). Also configured to export OTEL traces to `host.docker.internal:4318` (the `otel-collector` in the sibling `server-observability` stack).
...
- **Traefik**: `searxng`, `openwebui`, `litellm`, and `n8n` are dual-homed on `default` and the external `traefik_internal` network with `traefik.*` labels for a `*.oskarrosen.io` route (Traefik itself runs in `home-server-tools/traefik`; `traefik_internal` is `external: true`). `searxng`, `openwebui`, and `litellm` are LAN-only: `entrypoints=web,web_secure` with TLS like every router, plus `middlewares=lan-only@file` - DNS alone doesn't keep them private, since anyone reaching the public IP can send any `Host` header. None of them publish host ports (that would bypass Traefik and SSO). `n8n` is the one public service (Route53 record, `web_secure` only, `fail2ban-strict@file,fail2ban-404@file`). See `home-server-tools/traefik/README.md`.
...
Every service with credentials has a `<service>.env` (or `.env`) file referenced via `env_file:`, gitignored, with a committed `.env.example` sibling listing the same keys blanked out (`n8n.env`, `n8n-sandbox.env`, `openwebui.env`, `searxng.env`, `litellm.env`, `.env`) - keep the real files mode `0600`. When adding a new service that needs secrets, add both the real file and its `.example` counterpart.
