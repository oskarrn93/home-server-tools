...
- **Traefik**: `searxng`, `openwebui`, `litellm`, and `n8n` are dual-homed on `default` (`litellm` also on `media-services_default`) and the external `traefik_internal` network with `traefik.*` labels for a `*.oskarrosen.io` route — one hostname namespace, resolved to `192.168.1.12` by UniFi's local DNS for LAN clients, with a subset also getting a public AWS Route53 record. `searxng`, `openwebui`, and `litellm` stay LAN-only (`web` entrypoint, no TLS, no Route53 record); `n8n` is the one service in this repo meant to be reachable off-LAN, so it additionally uses the `web_secure` entrypoint with TLS via `lets_encrypt` and has a Route53 record. See `home-server-tools/traefik/README.md` for the public/LAN-only breakdown. Traefik itself runs elsewhere (sibling stack) — `traefik_internal` is `external: true`.
