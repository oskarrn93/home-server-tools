...
DNS alone does not make a host LAN-only: Traefik routes on the `Host` header, so anyone who reaches `:443` on the public IP can ask for any hostname, Route53 record or not. Every LAN-only router therefore lists `lan-only@file` (an `ipAllowList` for `192.168.1.0/24`, the router's VPN client subnets `192.168.2.0/24`/`192.168.3.0/24`/`192.168.6.0/24`, the Docker bridges, and loopback — see `dynamic/middlewares.yml`) first in its `middlewares`. Public routers leave it off. Services behind Traefik also shouldn't publish their own host ports (`ports:`), since Docker's iptables rules bypass UFW and a direct port skips `lan-only`, Tinyauth, and OIDC entirely — the remaining exceptions are bound to `127.0.0.1`/`172.17.0.1` (Ollama, Traefik's metrics) or harmless (OpenSpeedTest's `3000`, so speed tests aren't measured through the proxy).
